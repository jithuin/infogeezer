...
BOOL CEnBitmap::LoadImage(UINT uIDRes, LPCTSTR szResourceType, HMODULE hInst, COLORREF crBack)
{
	...
	return Attach(LoadImageResource(uIDRes, szResourceType, hInst, crBack == -1 ? m_crBkgnd : crBack, m_sizeDest.cx, m_sizeDest.cy));
}
...
BOOL CEnBitmap::LoadImage(LPCTSTR szImagePath, COLORREF crBack)
{
	...
	return Attach(LoadImageFile(szImagePath, crBack, m_sizeDest.cx, m_sizeDest.cy));
}
