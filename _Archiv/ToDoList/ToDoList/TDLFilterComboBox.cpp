...
void CTDLFilterComboBox::FillCombo()
{
	...
	if (m_bShowDefaultFilters)
	{
		...
		for (int nItem = 0; nItem < NUM_SHOWFILTER; nItem++, cLetter++)
		{
			CEnString sFilter(SHOW_FILTERS[nItem][0]);
			UINT nFilter = SHOW_FILTERS[nItem][1];
			
			if (cLetter > 'Z')
				sText.Format(_T("\t%s"), sFilter); 
			else
				sText.Format(_T("%c)\t%s"), cLetter, sFilter); 
			
			CDialogHelper::AddString(*this, sText, nFilter);
		}
	}
...
void CTDLFilterComboBox::AddCustomFilters(const CStringArray& aFilters)
{
