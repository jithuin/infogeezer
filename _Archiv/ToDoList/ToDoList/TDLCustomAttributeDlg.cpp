...
CTDLCustomAttributeDlg::CTDLCustomAttributeDlg(const CToDoCtrl& tdc, BOOL bMetroStyle, CWnd* pParent /*=NULL*/)
	: 
	CDialog(CTDLCustomAttributeDlg::IDD, pParent), 
	m_eTaskfile(FES_NOBROWSE), 
	m_bMetroStyle(bMetroStyle),
...
void CTDLCustomAttributeDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_SORTABLE, m_bSortable);
...
BEGIN_MESSAGE_MAP(CTDLCustomAttributeDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_SORTABLE, OnToggleSortable)
...
BOOL CTDLCustomAttributeDlg::OnInitDialog() 
{
	...
	m_lcAttributes.InsertColumn(COL_SORTABLE, CEnString(IDS_CAD_COLSORTABLE), LVCFMT_LEFT, 75);
...
BOOL CTDLCustomAttributeDlg::InitializeToolbar()
{
	...
	UINT nImageID = m_bMetroStyle ? IDB_CUSTATTRIB_TOOLBAR_METRO : IDB_CUSTATTRIB_TOOLBAR_STD;
...
int CTDLCustomAttributeDlg::AddAttributeToListCtrl(const TDCCUSTOMATTRIBUTEDEFINITION& attrib, BOOL bNew, int nPos)
{
	...
	CEnString sSortable(attrib.bSortable ? IDS_YES : IDS_NO);
	m_lcAttributes.SetItemText(nIndex, COL_SORTABLE, sSortable);
...
void CTDLCustomAttributeDlg::EnableControls()
{
	...
	GetDlgItem(IDC_SORTABLE)->EnableWindow(nSel >= 0);
...
void CTDLCustomAttributeDlg::OnToggleSortable() 
{
	UpdateData();
	int nSel = GetCurSel();
	...
	// update attribute
	TDCCUSTOMATTRIBUTEDEFINITION& attrib = m_aAttrib[nSel];
	attrib.bSortable = m_bSortable;

	// and list
	CEnString sSortable(attrib.bSortable ? IDS_YES : IDS_NO);
	m_lcAttributes.SetItemText(nSel, COL_SORTABLE, sSortable);
}
