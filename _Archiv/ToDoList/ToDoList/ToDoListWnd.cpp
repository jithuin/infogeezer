...
void CToDoListWnd::SetUITheme(const CString& sThemeFile)
{
	...
	// update the UI
	if (themeCur.nImageStyle != m_theme.nImageStyle)
	{
...
void CToDoListWnd::InitMenuIconManager()
{
	...
	if (CThemed::IsThemeActive())
	{
		...
		{
		case UIIS_GRAYSCALE:
			//nImageID = IDB_APP_TOOLBAR_GRAY;
			break;
						
		case UIIS_METRO:
			nToolbarImageID = IDB_APP_TOOLBAR_METRO;
			nExtraImageID = IDB_APP_EXTRA_METRO;
			break;
...
BOOL CToDoListWnd::InitToolbar()
{
	...
	// colors
	UINT nImageID = IDB_APP_TOOLBAR_STD;
	...
	if (CThemed::IsThemeActive())
	{
		...
		case UIIS_METRO:
			nImageID = IDB_APP_TOOLBAR_METRO;
			break;
...
LRESULT CToDoListWnd::OnToDoCtrlNotifyMod(WPARAM /*wp*/, LPARAM lp)
{
	...
	// reminders
	case TDCA_DELETE:
		m_reminders.RemoveDeletedTaskReminders(&GetToDoCtrl(nSel));
		break;
	}
...
void CToDoListWnd::OnContextMenu(CWnd* pWnd, CPoint point) 
{
	...
	{
		if (tdc.PtInHeader(point))
			nMenuID = HEADERCONTEXT;

		else if (tdc.WantTaskContextMenu())
		{
			nMenuID = TASKCONTEXT;
...
TDC_FILE CToDoListWnd::OpenTaskList(LPCTSTR szFilePath, BOOL bNotifyDueTasks)
{
	...
	if (nOpen == TDCO_SUCCESS)
	{
		int nIndex = AddToDoCtrl(pTDC, &storageInfo);
		...
		if (bNotifyDueTasks)
			DoDueTaskNotification(pTDC, userPrefs.GetNotifyDueByOnLoad());
		
		// save checkout status
		if (userPrefs.GetAutoCheckOut())
			m_mgrToDoCtrls.SetLastCheckoutStatus(nIndex, pTDC->IsCheckedOut());
...
BOOL CToDoListWnd::DoDueTaskNotification(const CFilteredToDoCtrl* pTDC, int nDueBy)
{
	// check userPrefs
	if (nDueBy == -1)
	...
	// save intermediate tasklist to file as required
	LogIntermediateTaskList(tasks, pTDC->GetFilePath());
	...
	if (bRes)
	{
		...
		CEnString sTitle(IDS_DUETASKS_TITLE, pTDC->GetFriendlyProjectName());

		m_dlgNotifyDue.SetDisplayInBrowser(bHtmlNotify);
		m_dlgNotifyDue.DoModal(sTitle, sTempFile, this);
/*

		if (!m_dlgNotifyDue.GetSafeHwnd())
			m_dlgNotifyDue.Create(sTitle, sTempFile, this);
		else
		{
			m_dlgNotifyDue.SetWindowText(sTitle);
			m_dlgNotifyDue.ShowWindow(SW_SHOW);
			m_dlgNotifyDue.Navigate(sTempFile);
		}
*/
	}
...
{
	static CString sTitle(_T("ToDoList 6.7.7 Feature Release"));
...
{
	CString sVersion;
	sVersion.Format(_T("<b>%s</b> © AbstractSpoon 2003-13"), GetTitle());
...
BOOL CToDoListWnd::ProcessStartupOptions(const TDCSTARTUP& startup)
{
	...
	if (startup.HasFilePath())
	{
		...
		for (int nFile = 0; nFile < nNumFiles; nFile++)
		{
			...
			else if (OpenTaskList(sFilePath, FALSE) == TDCO_SUCCESS)
				bRes++;
		}
...
void CToDoListWnd::SetTimer(UINT nTimerID, BOOL bOn)
{
	if (bOn)
	{
		...
		case TIMER_AUTOMINIMIZE:
			nPeriod = Prefs().GetAutoMinimizeFrequency() * 60000;
			break;
		}
...
void CToDoListWnd::OnTimerDueItems(int nCtrl)
{
	...
	SEARCHPARAMS spDue;
	spDue.aRules.Add(SEARCHPARAM(TDCA_DUEDATE, FO_BEFORE, dToday));
	spDue.aRules.Add(SEARCHPARAM(TDCA_DONEDATE, FO_NOT_SET, dToday)); 

	SEARCHPARAMS spDueToday;
	spDueToday.aRules.Add(SEARCHPARAM(TDCA_DUEDATE, FO_EQUALS, dToday));
	spDueToday.aRules.Add(SEARCHPARAM(TDCA_DONEDATE, FO_NOT_SET, dToday)); 
...
void CToDoListWnd::OnTimerAutoMinimize()
{
	...
	if (GetLastInputInfo(&lii))
	{
		...
		dElapsed /= 60000; // convert to minutes
...
void CToDoListWnd::OnFileSaveToUserStorage(UINT nCmdID) 
{
	...
	if (sLocalPath.IsEmpty())
	{
		...
		if (sLocalPath.IsEmpty())
			sLocalPath = FileMisc::GetTempFileName(_T("tdt"));
	}
...
void CToDoListWnd::OnSelchangeTabcontrol(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
{
	...
	if (nCurSel != -1)
	{
		...
		// notify user of due tasks if req
		DoDueTaskNotification(&tdc, nDueBy);
...
void CToDoListWnd::OnExport() 
{
	...
	CTDLExportDlg dialog(m_mgrImportExport, nTDCCount == 1, GetToDoCtrl().GetView(),
...
void CToDoListWnd::OnEditSetReminder() 
{
	...
	CDWordArray aTaskIDs;
	int nNumSel = tdc.GetSelectedTaskIDs(aTaskIDs);
...
void CToDoListWnd::OnEditClearReminder() 
{
	...
	CDWordArray aTaskIDs;
	int nTask = tdc.GetSelectedTaskIDs(aTaskIDs);
...
void CToDoListWnd::OnTasklistCustomColumns() 
{
	...
	if (!tdc.IsReadOnly())
	{
		BOOL bMetroStyle = (m_theme.nImageStyle == UIIS_METRO);

		CTDLCustomAttributeDlg dialog(tdc, bMetroStyle);
