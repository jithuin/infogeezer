...
#define TASKS_FONT_NAME			_T("Tahoma")
...
CTransparentListBox::CTransparentListBox(int nCellFontSize)
	...
	m_bCompletedItemsStrikethru(FALSE)
{
	...
	GraphicsMisc::CreateFont(m_font, TASKS_FONT_NAME, nCellFontSize/*, MFS_BOLD*/);
	GraphicsMisc::CreateFont(m_fontStrikethru, TASKS_FONT_NAME, nCellFontSize, /*MFS_BOLD|*/GMFS_STRIKETHRU);
}
...
void CTransparentListBox::DoPaint(CDC* pDC)
{
	CRect rc;
	GetClientRect(&rc);
	...
	dcMem.CreateCompatibleDC(pDC);
	CBitmap bmpMem;
	bmpMem.CreateCompatibleBitmap(pDC, nWidth, nHeight);
	...
	for (int i = GetTopIndex(); (i < nSize) && (rc.top < nHeight); i++)
	{
		...
		rc.bottom = max(rc.bottom, nHeight);
	}
...
void CTransparentListBox::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	pDC->DrawText("XYZ", &rc, DT_CALCRECT|DT_EXPANDTABS|DT_NOPREFIX);
...
void CTransparentListBox::DrawItem(CDC& _dc,
								   int _nIndex,
								   CRect& _rc,
								   BOOL _bSelected)
{
	...
	CBitmap* pbmpOld = dcMem.SelectObject(&m_bmp);
	_dc.BitBlt(rcItem.left, rcItem.top, rcItem.Width(), rcItem.Height(),
			   &dcMem, rcItem.left, rcItem.top, SRCCOPY);
...
int CTransparentListBox::AddString(const CString& _strText,
								   DWORD _dwItemData/*=0*/,
...
int CTransparentListBox::InsertString(int _nIndex,
									  const CString& _strText,
									  DWORD _dwItemData/*=0*/,
									  BOOL _bCompleted/*=FALSE*/)
{
	SetRedraw(FALSE);
...
void CTransparentListBox::SetCompletedItemDisplayStyle(BOOL _bGrey,
													   BOOL _bStrikethru)
{
