...
BEGIN_MESSAGE_MAP(CRulerRichEditCtrl, CWnd)
	...
	//}}AFX_MSG_MAP
//	ON_WM_PAINT()
	ON_COMMAND(BUTTON_INSERTTABLE, OnButtonInsertTable)
	ON_COMMAND(BUTTON_JUSTIFY, OnButtonJustify)
	...
	ON_COMMAND(BUTTON_SHRINKFONT, OnButtonShrinkFont)
	ON_COMMAND(BUTTON_GROWFONT, OnButtonGrowFont)
	ON_WM_CREATE()
...
void CRulerRichEditCtrl::OnButtonJustify() 
{
	DoJustify();
}

void CRulerRichEditCtrl::OnButtonShrinkFont() 
{
	IncrementFontSize(FALSE);
}

void CRulerRichEditCtrl::OnButtonGrowFont() 
{
...
void CRulerRichEditCtrl::OnButtonInsertTable() 
{
	DoInsertTable();
}
   ...
   ============================================================*/
{
	if (m_showToolbar && m_toolbar.m_hWnd)
	{
		...
		m_toolbar.CheckButton(BUTTON_JUSTIFY, (para.wAlignment == PFA_JUSTIFY));
