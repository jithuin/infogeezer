...
namespace PInvokeLibrary
{
	...
	public class Registry
	{
		...
		/*public static void TestProc(MainTest.DisplayLineDelegate showLine)
		{
			...
			string stringValueName = @"MyStringValue";
			string DWORDValueName = @"MyDwordValue";
			string stringData = @"MyStringData";
			uint DWORDData = 0xABCDEF01;
			uint resultDWORDData = 0x00000000;
			string resultStringData = "Empty";

			showLine(string.Format("Creating key {0}", keyName));
			int result = CreateKey(keyName);
			if (result != Registry.ERROR_SUCCESS)
			{
				showLine("FAILURE: Failed to create key");
				return;
			}

			showLine(string.Format("Creating DWORD value 0x{0:X8}", DWORDData));
			result = CreateValueDWORD(keyName, DWORDValueName, DWORDData);
			if (result != Registry.ERROR_SUCCESS)
				showLine("FAILURE: Failed to create value");

			showLine(string.Format("Creating string value {0}", stringData));
			result = CreateValueString(keyName, stringValueName, stringData);
			if (result != Registry.ERROR_SUCCESS)
				showLine("FAILURE: Failed to create value");

			showLine("Getting DWORD value");
			result = GetDWORDValue(keyName, DWORDValueName, ref resultDWORDData);
			if (result != Registry.ERROR_SUCCESS)
			{
				showLine("FAILURE: Failed to get value");
			}
			else
			{
				if (resultDWORDData == DWORDData)
					showLine(string.Format("Returned proper value: 0x{0:X8}", DWORDData));
				else
					showLine(string.Format("FAILURE: Set:0x{0:X8} Get:0x{1:X8}", DWORDData, resultDWORDData));
			}

			showLine("Getting string value");
			result = GetStringValue(keyName, stringValueName, ref resultStringData);
			if (result != Registry.ERROR_SUCCESS)
			{
				showLine("FAILURE: Failed to get value");
			}
			...
			{
				if (resultStringData == stringData)
					showLine(string.Format("Returned proper value: {0}", stringData));
				else
					showLine(string.Format("FAILURE: Set:{0} Get:{1}", stringData, resultStringData));
			}

			showLine("Deleting DWORD value");
			result = DeleteValue(keyName, DWORDValueName);
			if (result != Registry.ERROR_SUCCESS)
				showLine("FAILURE: Failed to delete value");

			showLine("Deleting string value");
			result = DeleteValue(keyName, stringValueName);
			if (result != Registry.ERROR_SUCCESS)
				showLine("FAILURE: Failed to delete value");

			showLine("Deleting key");
			result = DeleteKey(keyName);
			if (result != Registry.ERROR_SUCCESS)
				showLine("FAILURE: Failed to delete key");
		}*/
	}
}
