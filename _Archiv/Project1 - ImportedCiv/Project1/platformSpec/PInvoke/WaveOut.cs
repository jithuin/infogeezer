	...
	/*	public static void TestProc(MainTest.DisplayLineDelegate showLine)
		...
		{ 
			...
			{ 
				...
				if ( numDevices < 1 ) 
				{ 
					MessageBox.Show( "No sound device detected", "Error" ); 
					return; 
				} 
				/*String fileName = System.Reflection.Assembly.GetExecutingAssembly().GetName().CodeBase; 
				fileName = Path.GetDirectoryName(fileName); 
				...
				if ( Wave.MMSYSERR.NOERROR != wo.Play(file, 512*1024, 0xffff, 0xffff) ) 
				{ 
					MessageBox.Show( "Failed to play the file: " + file, "Error" ); 
				} 
				else
				{
					uint soundLength = wo.Milliseconds();
